...
class LinearRegressionModel():
    ...
    def data_preprocessing(self, csv_file):
    ...
    def get_daily_values(self, dataframe):
    ...
    def data_preparation(self, dataframe):
        ...
        averaged_dataframe = self.get_daily_values(dataframe)
    ...
    ''' formatted_test_model_predictions function is used to test out
        ...
        for readability'''
    def formatted_test_model_predictions(self, model, x_test):
    ...
    def get_monthly_predictions(self, model, features):
        ...
        monthly_predictions_df = self.format_dataframe(predictions_df)
...
if __name__ == "__main__":
    ...
    if os.path.isfile(PC_CSV):
        power_model = LinearRegressionModel()
        processed_data = power_model.data_preprocessing(PC_CSV)
        x_train, x_test, y_train, y_test, features = power_model.data_preparation(processed_data)
        # Check if the model exists
        if os.path.isfile(MODEL_PATH):
            power_model.get_monthly_predictions(MODEL_PATH, features)
        else:
            power_model.model_training(x_train, x_test, y_train, y_test)
            power_model.get_monthly_predictions(MODEL_PATH, features)
    else:
