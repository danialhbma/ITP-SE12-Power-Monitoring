...
class GrowlightOnAlert(Alert):
    ...
    def _evaluate_alert(self, wattage:int) -> AlertState:
        ...
        try:
            if (wattage < 30): # wattage will be higher than 30 if recently switched on 
                return AlertState.OK
            if (wattage >= self.threshold):
                return AlertState.ALERTING
            elif (wattage < self.threshold):
                return AlertState.OK
            else:
    ...
    def _process_query_results(self, query_result):
        ...
        try:
            ...
            print("growlight on alert: ", value, sg_time)
...
class GrowlightOnAlertMessage(AlertMessage):
    ...
    def _populate_message_elements(self, alert_state: AlertState, time, exception = ""):
        ...
        if alert_state == AlertState.ALERTING:
            self.summary += f"{self.location} switched on."
            self.description += f"{self.location} switched on at {time}."
            return 
